...
with open(filename, 'r') as fp:
    for line in fp.readlines():
        stride, N, size, total_time, time_per_it, cy_per_gather, cy_per_elem  = line.split(',')
        stride = int(stride)
        size = float(size)
        cy_per_gather = float(cy_per_gather)

        if stride not in plot_data:
            plot_data[stride] = {}

        plot_data[stride][size] = cy_per_gather if size not in plot_data[stride] \
                                  else min(cy_per_gather, plot_data[stride][size])
